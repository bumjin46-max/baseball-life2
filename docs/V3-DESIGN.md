...
1. **주간 시스템** — `buildMonth()`가 `['action']` 대신 4개를 돌려주면 그대로 주간 4칸이 됩니다. UI만 4칸으로 그리면 끝입니다.
2. **구단 이벤트 5~10개/구단** (요구 19) — 현재 2~4개 + 문화 공용 4종.
3. **외국인 선수 · 지명권 트레이드 · 시즌 중 트레이드** — v2.1부터 미구현으로 남아 있는 항목.
4. **React + Vite 이전** — 파일 7개가 `calendar.ts` / `lifeEngine.ts` / `leagueEngine.ts` 등으로 1:1 대응됩니다.
